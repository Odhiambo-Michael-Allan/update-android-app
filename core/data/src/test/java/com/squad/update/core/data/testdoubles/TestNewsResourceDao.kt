...
class TestNewsResourceDao : NewsResourceDao {
    ...
    override fun getNewsResources(
        ...
        entitiesStateFlow
            .map { newsResourceEntities ->
                ...
                var result = populatedNewsResources
                ...
                if ( useFilterNewsIds ) {
                    result = result.filter { it.entity.id in filterNewsIds }
                }
                result
            }
    ...
    override fun getNewsResourceIds(
        ...
        .map { newsResourceEntities ->
            newsResourceEntities.map { newsResourceEntity ->
                newsResourceEntity.asPopulatedNewsResource( topicCrossReferences )
            }
        }.map { populatedNewsResources ->
            var result = populatedNewsResources
            if ( useFilterTopicIds ) {
                result = result.filter { populatedNewsResource ->
    ...
    override suspend fun upsertNewsResources(
